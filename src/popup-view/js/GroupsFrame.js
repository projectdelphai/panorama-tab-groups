...
async function _renderGroupList() {
    ...
    const groupNodes = await Promise.all(groups.map(_renderGroupListItem.bind(this)));
    ...
    const node = getElementNodeFromString(`
        <li id="group-${Group.id}" class="list__item ${isActive ? 'list__item--highlight' : ''}" data-nav-row>
            ...
            <button class="list__link list__link--extend">
                <span>
                    ${getPluralForm(tabCount, browser.i18n.getMessage('tabCount', [tabCount]))}
                </span>
            </button>
        </li>
    `);
...
function _enableGroupDragAndDrop() {
    ...
    this.listItems.forEach((listItem) => {
        ...
        listItem.addEventListener('dragstart', _handleGroupDragStart.bind(this), false);
...
function _addOrUpdateDropZoneHandler() {
    ...
    this.listItems.forEach((listItem, index) => {
        ...
        newDropZone.addEventListener('dragenter', _handleGroupDragEnter.bind(this), false);
        newDropZone.addEventListener('dragleave', _handleGroupDragLeave.bind(this), false);
