...
// primitive mutex to make sure the functions that deal
// with groups aren't stepping on each other's toes
    ...
    groups.forEach((_group) => {
      if (_group.id !== group.id) {
        if (top && bottom) {
          ...
          rect.j = snapValue(rect.j, _group.rect.y, snapDsty);
          rect.j = snapValue(rect.j, _group.rect.y + _group.rect.h, snapDsty);
        }
        ...
        if (left && right) {
          ...
          rect.i = snapValue(rect.i, _group.rect.x, snapDstx);
          rect.i = snapValue(rect.i, _group.rect.x + _group.rect.w, snapDstx);
        }
      }
    });
    // ----
...
export function makeGroupNode(group) {
  ...
  // corners
  const topRight = newElement('div', { class: 'topRight' });
  const bottomRight = newElement('div', { class: 'bottomRight' });
  const bottomLeft = newElement('div', { class: 'bottomLeft' });
  const topLeft = newElement('div', { class: 'topLeft' });
